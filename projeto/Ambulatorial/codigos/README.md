...
spark = SparkSession.builder.appName("ml_ambulatorial").getOrCreate()

# Tabelas principais do Lakehouse
F_AMBULATORIAL = "F_AMBULATORIAL"
D_PACIENTE = "D_PACIENTE"
D_REGIAO = "D_REGIAO"
D_TEMPO = "D_TEMPO"
D_CID = "D_CID"
D_PROCEDIMENTO = "D_PROCEDIMENTO_AMB"

# Variável alvo
TARGET = "VALOR_PROCEDIMENTO"
...
f_amb = spark.table(F_AMBULATORIAL)
d_pac = spark.table(D_PACIENTE)
d_reg = spark.table(D_REGIAO)
d_temp = spark.table(D_TEMPO)
d_cid = spark.table(D_CID)
d_procedimento = spark.table(D_PROCEDIMENTO)

# Junção com dimensões
df = (
    f_amb
    .join(d_pac, f_amb["D_PACIENTE_idD_PACIENTE"] == d_pac["idD_PACIENTE"], "left")
    .join(d_reg, f_amb["D_REGIAO_idD_REGIAO"] == d_reg["idD_REGIAO"], "left")
    .join(d_temp, f_amb["D_TEMPO_idD_TEMPO"] == d_temp["idD_TEMPO"], "left")
    .join(d_cid, f_amb["D_CID_idD_CID"] == d_cid["idD_CID"], "left")
    .join(d_procedimento, f_amb["D_PROCEDIMENTO_AMB_idD_PROCEDIMENTO_AMB"] == d_procedimento["idD_PROCEDIMENTO"], "left")
)

df.select("ano", "faixa_etaria", "idade_media", "cid10", "nome_municipio", "nome_procedimento", "valor_procedimento").show(5)
...
df_agg = (
    df.groupBy("ano", "nome_municipio", "faixa_etaria", "cid10", "nome_procedimento")
      .agg(
          F.count("*").alias("qtde_Atendimentos"),
          F.sum("valor_procedimento").alias("valor_procedimento"),
          F.avg("idade_media").alias("idade_media")
      )
)

# Proporção de internações por faixa etária (proxy de envelhecimento)
total_ano = df_agg.groupBy("ano").agg(F.sum("qtde_Atendimentos").alias("total_ano"))
df_agg = df_agg.join(total_ano, "ano").withColumn(
    "prop_faixa_etaria", F.col("qtde_Atendimentos") / F.col("total_ano")
)
...
# Define variáveis
numeric_features = ["valor_procedimento", "idade_media", "prop_faixa_etaria", "prop_60plus", "qtde_Atendimentos"]
categorical_features = ["nome_municipio", "faixa_etaria", "cid10", "nome_procedimento"]
...
# Modelos
lr = LinearRegression(labelCol="valor_procedimento", featuresCol="scaled_features")
rf = RandomForestRegressor(labelCol="valor_procedimento", featuresCol="scaled_features", numTrees=100, maxDepth=8)
gbt = GBTRegressor(labelCol="valor_procedimento", featuresCol="scaled_features", maxIter=100, maxDepth=5)
...
train_df = df_ml.filter(F.col("ano") <= 2022)
test_df = df_ml.filter(F.col("ano") > 2022)

evaluator = RegressionEvaluator(labelCol="valor_procedimento", predictionCol="prediction", metricName="rmse")
...
for name, pipeline in pipelines.items():
    ...
    r2 = RegressionEvaluator(labelCol="valor_procedimento", predictionCol="prediction", metricName="r2").evaluate(preds)

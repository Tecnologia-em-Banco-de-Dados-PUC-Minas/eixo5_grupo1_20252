...
spark = SparkSession.builder.appName("ml_mortalidade").getOrCreate()

# Tabelas principais do Lakehouse
F_MORTALIDADE = "F_MORTALIDADE"
...
f_mort = spark.table(F_MORTALIDADE)
...
df = (
    f_mort
    .join(d_pac, f_mort["D_PACIENTE_idD_PACIENTE"] == d_pac["idD_PACIENTE"], "left")
    .join(d_reg, f_mort["D_REGIAO_idD_REGIAO"] == d_reg["idD_REGIAO"], "left")
    .join(d_temp, f_mort["D_TEMPO_idD_TEMPO"] == d_temp["idD_TEMPO"], "left")
    .join(d_cid, f_mort["D_CID_idD_CID"] == d_cid["idD_CID"], "left")
)

df.select("ano", "faixa_etaria", "idade_media", "cid10", "nome_municipio", "qtde_Obitos").show(5)
...
df_agg = (
    df.groupBy("ano", "nome_municipio", "faixa_etaria", "cid10")
      .agg(
          F.sum("qtde_Obitos").alias("qtde_Obitos"),
          F.avg("idade_media").alias("idade_media")
      )
)

# Proporção de internações por faixa etária (proxy de envelhecimento)
total_ano = df_agg.groupBy("ano").agg(F.sum("qtde_Obitos").alias("total_ano"))
df_agg = df_agg.join(total_ano, "ano").withColumn(
    "prop_faixa_etaria", F.col("qtde_Obitos") / F.col("total_ano")
)
...
# Define variáveis
numeric_features = ["qtde_Obitos", "idade_media", "prop_faixa_etaria", "prop_60plus"]
...
# Modelos
lr = LinearRegression(labelCol="qtde_Obitos", featuresCol="scaled_features")
rf = RandomForestRegressor(labelCol="qtde_Obitos", featuresCol="scaled_features", numTrees=100, maxDepth=8)
gbt = GBTRegressor(labelCol="qtde_Obitos", featuresCol="scaled_features", maxIter=100, maxDepth=5)
...
evaluator = RegressionEvaluator(labelCol="qtde_Obitos", predictionCol="prediction", metricName="rmse")
...
for name, pipeline in pipelines.items():
    ...
    r2 = RegressionEvaluator(labelCol="qtde_Obitos", predictionCol="prediction", metricName="r2").evaluate(preds)
